...
var vistedPages = [];
var pagesToVist = ["https://www.nandilathgmart.com/branches.html","https://www.nandilathgmart.com/branches.html?page=2","https://www.nandilathgmart.com/branches.html?page=3"];
...
async function scrape(currlink) {
    ...
    try {
        await page.waitForSelector('.details.cn-height');
    ...
    let tbleCont = await page.evaluate(() => {
        ...
        let tableHeader = document.querySelectorAll('.details.cn-height');
        ...
        if (tableHeader) {
            tableHeader.forEach((tr, i) => {
                if (tr) {
                    ...
                    if (tr.querySelector(".address")) {
                        item["address"] = tr.querySelector(".address").innerText;
                    }
                    if (tr.querySelector(".phone")) {
                         item["phone"] = tr.querySelector(".phone").innerText;
                    }
                    arr.push(item);
                }
