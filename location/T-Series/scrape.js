...
var vistedPages = [];
var pagesToVist = ["http://ced.tseries.com/branch-network.php"];
...
async function scrape(currlink) {
   ...
   // await page.screenshot({ path: pic, type: 'jpeg', fullPage: true });
    try {
        await page.waitForSelector('table table:nth-child(2)', { timeout: 1000 });
    ...
    let tbleCont = await page.evaluate(() => {
       ...
       /* item.title = document.querySelector(".ctl_aboutbrand h1").innerText;
        ...
        let tableHeader = document.querySelectorAll('table table:nth-child(2) tr td');
