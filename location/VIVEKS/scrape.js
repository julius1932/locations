...
var vistedPages = [];
var pagesToVist = ["https://www.viveksindia.com/"];
...
async function scrape(currlink) {
    ...
    for (let i = 0; i < states.length; i++) {
        ...
        await page.waitForSelector('a.strLink', { timeout: 40000 });
        await page.waitFor(4000);
        let cities = await page.evaluate(() => {
            var arr = [];
            let tableHeader = document.querySelectorAll('a.strLink');
            if (tableHeader) {
                tableHeader.forEach((tr, i) => {
                    if (tr) {
                        if (tr.innerText) {
        ...
        for (let k = 0; k < cities.length; k++) {
            await page.click(`a.strLink:nth-of-type(${1+k})`);
            await page.waitForSelector('.right', { timeout: 40000 });
            await page.waitFor(4000);
            let adresses = await page.evaluate(() => {
                var arr = [];
                let tableHeader = document.querySelectorAll('.right');
                if (tableHeader) {
                    tableHeader.forEach((tr, i) => {
                        if (tr) {
