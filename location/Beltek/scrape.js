...
var vistedPages = [];
var pagesToVist = ["http://beltek.co.in/branch-location/"];
...
async function scrape(currlink) {
    ...
    try {
        await page.waitForSelector('div.wpb_text_column.wpb_content_element', { timeout: 1000 });
    ...
    let tbleCont = await page.evaluate(() => {
        ...
        let tableHeader = document.querySelectorAll('div.wpb_text_column.wpb_content_element');

        if (tableHeader) {
            tableHeader.forEach((tr, i) => {
                if (tr) {
                    let item = {};
                    //item["title"] = tr.querySelector("h1").innerText;
                    item["other"] = tr.innerText;
                    if(tr.querySelector(".fa.fa-phone")){
                        item["contact"] = tr.querySelector(".fa.fa-phone").innerText; 
                    }
