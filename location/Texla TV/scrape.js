...
var vistedPages = [];
var pagesToVist = [];//["https://www.texlavision.com/where-to-buy/"];
for(let k=1;k<9;k++){
    pagesToVist.push(`https://www.texlavision.com/where-to-buy/?p=${k}&category=0&zoom=19&is_mile=0&directory_radius=5&view=grid&sort=title`);
}
...
async function scrape(currlink) {
    ...
    try {
        await page.waitForSelector('div.sabai-directory-main', { timeout: 1000 });
    ...
    let tbleCont = await page.evaluate(() => {
        ...
        let tableHeader = document.querySelectorAll('div.sabai-directory-main');

        if (tableHeader) {
            tableHeader.forEach((tr, i) => {
                if (tr) {
                    let item = {};
                    item["title"] = tr.querySelector(".sabai-directory-title").innerText;
                    item["category"] = tr.querySelector(".sabai-directory-category").innerText;
                    item["location"] = tr.querySelector(".sabai-directory-location").innerText;
                    item["contact"] = tr.querySelector(".sabai-directory-contact").innerText;
